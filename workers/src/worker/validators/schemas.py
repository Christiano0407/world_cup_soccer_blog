...
class CleanMatchesRow(BaseModel):
    ...
    def validator_initials(cls, v: str) -> str:
        ...
        if not re.match(TEAM_INITIALS_PATTERN, val_initials):
            raise ValueError(
                f"Iniciales Inválidas: '{val_initials}' — debe ser 2-3 letras (BRA)."
            )
        return val_initials
    ...
    def team_different(self) -> CleanMatchesRow:
        ...
        if self.home_team_initials == self.away_team_initials:
            raise ValueError(
                f"'home' & 'Away' no pueden ser el mismo equipo: {self.home_team_initials}"
            )
        return self
    ...
    def ht_both_goals_or_none(self) -> CleanMatchesRow:
        ...
        if (self.ht_home_goals is None) != (self.ht_away_goals is None):
            raise ValueError(
                "HT goles: ambos None o ambos int — nunca uno solo."
            )
        return self
...
class CleanPlayersRow(BaseModel):
    ...
    shirt_number: Annotated[int, Field(ge=1, le=MAX_SHIRT_NUMBER)] | None = (
        None  # "0" (Sin asignar en el CSV)  # noqa: E501
    )
    player_name: str
    position: str | None = None  # "GK", "DF", "MF", "FW"
    ...
    def validate_init_line_up(cls, v: str) -> str:
        ...
        if val_line_up is None:
            raise ValueError(
                f"Lineup type inválido: '{v}'. Debe ser: 'S' o 'N'."
            )
        return val_line_up
    ...
    def player_name_not_empty(cls, v: str) -> str:
        """Validar que player_name no esté vacío"""
        if not v or not v.strip():
