...
def validate_winners_row(
    ...
    if not host_country:
        errors.append(
            _err(
                "country",
                "MISSING_HOST_WINNER",
                "No tenemos campeón / No existe información del País Campeón del Mundo de ese año",
            )
        )
    ...
    if not third and not fourth:
        errors.append(
            _warn(
                "third/fourth",
                "MISSING_PODIUM",
                "Podium Incompleto: No tenemos ni tercer ni cuarto lugar.",
            )  # noqa: E501
        )
    ...
    if goals is None:
        errors.append(
            _err(
                "goals_scored",
                "EMPTY_GOALS_SCORED",
                f"No tiene goles anotados durante el torneo de fútbol mundial: {raw.goals_scored}",
            )
        )  # noqa: E501
    elif goals < 0:
        errors.append(
            _err(
                "goals_scored",
                "NEGATIVE_GOALS_SCORED",
                f"Goles negativos: goals_scored={goals} < 0",
            )
        )
    ...
    if qualified is None:
        errors.append(
            _err(
                "qualified_teams",
                "MISSING_QUALIFIED_TEAMS",
                f"Éste equipo no está Clasificado al Mundial. Clasificados: {raw.qualified_teams}",
            )
        )
        ...
        errors.append(
            _err(
                "qualified_teams",
                "NEGATIVE_QUALIFIED_TEAMS",
                f"Clasificadas debe estar entre 1 y 48: {qualified}",
            )
        )
    ...
    if matches is None:
        errors.append(
            _err(
                "matches_played",
                "MISSING_MATCHES_PLAYED",
                f"No tenemos registro de partidos jugados: {raw.matches_played}",
            )
        )
    elif matches < 1:
        errors.append(
            _err(
                ...
                f"Partidos jugados inválidos: {matches} — mínimo 1 (o 3 en fase de grupos)",
            )
        )
    ...
    if raw.attendance and attendance is None:
        errors.append(
            _warn(
                "attendance",
                "UNPARSEABLE_ATTENDANCE",
                f"Attendance no parseable: {raw.attendance} - se guardará como NULL",
            )
        )  # noqa: E501
        ...
        if avg < 0.3:
            errors.append(
                _warn(
                    "goals_scored",
                    "LOG_AVG_MATCHES",
                    f"avg={avg:.2f} goles/partidos - parece bajo (nivel de anotaciones. Verificar)",
                )
            )  # noqa: E501
        if avg > 8.0:
            errors.append(
                _warn(
                    "goals_scored",
                    "HIGH_SCORED_GOALS",
                    f"avg={avg:.2f} g/partido — alto nivel de anotaciones. Verificar",
                )
            )
    ...
    return _valid(
        clean, warnings=[e for e in errors if e.severity == "warning"], raw_row_id=raw_row_id
    )  # noqa: E501
...
def validate_matches_row(
    ...
    if match_id is None or match_id < 1:
        errors.append(_err("match_id", "INVALID_MATCH_ID", f"MatchID inválido: '{raw.match_id}'"))

    # ---- tournament_id ---- #
    if tournament_id < 1:
        errors.append(
            _err(
                "tournament_id", "INVALID_TOURNAMENT_ID", f"TournamentID inválido: {tournament_id}"
            )
        )
    ...
    if round_id is None or round_id < 1:
        errors.append(_err("round_id", "INVALID_ROUND_ID", f"RoundID inválido: '{raw.round_id}'"))

    # ---- year ---- #
    year = parse_int(raw.year)
    if year is None:
        errors.append(_err("year", "MISSING_YEAR", f"Year vacío o no parseable: '{raw.year}'"))
    elif not (MIN_WC_YEAR <= year <= MAX_WC_YEAR):
        errors.append(
            _err(
                "year",
    ...
    if home_goals is None or home_goals < 0:
        errors.append(
            _err(
                "home_goals", "INVALID_HOME_GOALS", f"Home goals inválido: '{raw.home_team_goals}'"
            )
        )
    ...
    if away_goals is None or away_goals < 0:
        errors.append(
            _err(
                "away_goals", "INVALID_AWAY_GOALS", f"Away goals inválido: '{raw.away_team_goals}'"
            )
        )
...
def validate_players_row(
    ...
    if not player_name:
        errors.append(_err("player_name", "MISSING_PLAYER_NAME", "Player name vacío"))

    # ---- position ---- #
    position = normalize_player_position(raw.position)
    if raw.position and not position:
        errors.append(
            _warn("position", "UNKNOWN_POSITION", f"Posición no reconocida: '{raw.position}'")
        )
    ...
    if raw.event and not event_code:
        errors.append(
            _warn("event_code", "UNKNOWN_EVENT_CODE", f"Event code no reconocido: '{raw.event}'")
        )
...
def validate_team_row(
    ...
    Los equipos se extraen de team_initials/home_team_initials/away_team_initials.
    """
    errors: list[ValidationError] = []
...
def validate_round_row(
    ...
    if parsed_round_id is None or parsed_round_id < 1:
        errors.append(_err("round_id", "INVALID_ROUND_ID", f"RoundID inválido: '{round_id}'"))
    ...
    if not validated_round_name:
        errors.append(_err("round_name", "MISSING_ROUND_NAME", "Nombre de ronda vacío"))
