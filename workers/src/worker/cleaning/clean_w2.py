...
class CleanResults:
    ...
    def log_summary(self) -> None:
        ...
        log.info(
            ...
            top_errors=sorted(self.error_breakdown.items(), key=lambda x: -x[1])[:5],
        )
...
async def clean_dataset(dataset: DatasetKind, settings: Settings) -> CleanResults:
    ...
    try:
        match dataset:
            ...
            case _:
                raise ValueError(
                    f"Dataset desconocido: '{dataset}'  - debe ser winners | matches | players "
                )
    finally:
        await pool.close()
...
async def _clean_winners(pool: asyncpg.Pool) -> CleanResults:
    result = CleanResults(
        dataset="winners", rows_checked=0, rows_valid=0, rows_rejected=0, rows_with_warning=0
    )
...
async def _clean_matches(pool: asyncpg.Pool) -> CleanResults:
    result = CleanResults(
        dataset="matches", rows_checked=0, rows_valid=0, rows_rejected=0, rows_with_warning=0
    )
    ...
    async for batch in _fetch_batches(pool, "raw.wc_matches"):
        ...
        for row in batch:
            ...
            validation = validate_matches_row(
                raw, tournament_id=tournament_id, raw_row_id=row["_row_id"]
            )
...
async def _clean_players(pool: asyncpg.Pool) -> CleanResults:
    result = CleanResults(
        dataset="players", rows_checked=0, rows_valid=0, rows_rejected=0, rows_with_warning=0
    )
